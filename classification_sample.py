...
def classification_sample(num_units, learning_rate, swap):
    """ Run a Decision Network Classification with parameters set outside
    - does a training after every sample
    ...
    for idx_seed in range(num_seeds):
        ...
        for idx_episode in range(num_episode_train):
            ...
            for idx_trial in range(num_trial_per_episode):
                ...
                # train the network using the current target and input
                target_3d = np.array([[[target]]])
                info = dm_network.fit(dm_input, target_3d, verbose=0)
                current_loss = info.history.get('loss', '')

                # store loss, choice, prediction and target
                train_loss_matrix[idx_seed, idx_episode, idx_trial] = current_loss[0]
                choice_matrix[idx_seed, idx_episode, idx_trial] = choice
                prediction_matrix[idx_seed, idx_episode, idx_trial] = prediction
                target_matrix[idx_seed, idx_episode, idx_trial] = target
    ...
    month = today.month
    day = today.day
    filename = 'Class_sample_{0}{1}_nEpisode:{2}_nUnits:{3}_learning_rate:{4}_swap:{5}.pickle'\
