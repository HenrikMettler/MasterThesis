...
class Worker():
    def __init__(self, game, name, a_size, trainer, model_path, global_episodes, make_gif=False):
        ...
        self.local_AC = AcNetwork(a_size, self.name, trainer)
        self.update_local_ops = update_target_graph('global', self.name)
        self.env = game
        self.make_gif = make_gif

    def train(self, episodeBuffer, sess, gamma, bootstrap_value):
        episodeBuffer = np.array(episodeBuffer)
        states = episodeBuffer[:, 0]
        actions = episodeBuffer[:, 1]
        rewards = episodeBuffer[:, 2]
        timesteps = episodeBuffer[:, 3]
        prev_rewards = [0] + rewards[:-1].tolist()
        prev_actions = [0] + actions[:-1].tolist()
        values = episodeBuffer[:, 5]
        ...
        v_l, p_l, e_l, g_n, v_n, _ = sess.run([self.local_AC.value_loss,
                                              ...
                                              feed_dict=feed_dict)
        return v_l / len(episodeBuffer), p_l / len(episodeBuffer), e_l / len(episodeBuffer), g_n, v_n
