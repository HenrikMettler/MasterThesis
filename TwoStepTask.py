# Source: https://github.com/mtrazzi/two-step-task
import numpy as np
from helper import *
...
class TwoStepTask():
    def __init__(self, switch_likelihood, high_reward_probs):
        # "New" variables
        self.switch_likelihood = switch_likelihood
        self.highest_reward_probs = high_reward_probs
    ...
    def possible_switch(self):
        if (np.random.uniform() < self.switch_likelihood):
            # switches which of S_2 or S_3 has expected reward of high_reward_probs
    ...
    def get_rprobs(self):
        ...
        if (self.highest_reward_second_stage == S_2):
            r_prob = self.highest_reward_probs
        else:
            r_prob = 1 - self.highest_reward_probs
    ...
    def step(self, action):
        ...
        if (self.state == S_1):
            ...
            r_prob = self.highest_reward_probs if (self.highest_reward_second_stage == self.state) \
                else 1 - self.highest_reward_probs
            # get reward
    ...
    def trial(self, action):
        # do one action in S_1, and keep track of the perceptually distinguishable state you arrive in
